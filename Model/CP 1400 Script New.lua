...
-- Sobrevelocidade a paragem absoluta
	...
	if dstate == 1 and ddistance < 1 then
		kstart = 1
		ktime = 0
	end
	
ktime = ktime + time
kdistance = kdistance - sdistance
	if kstart == 1 and ktime > 1 and dstate == 2 then
		kdistance = ddistance
		sdistance = 0
		kstart = 0
		ksignal = 1
	elseif kstart == 1 and ktime > 1 and dstate ~= 2 then
		kstart = 0
		ksignal = 0		
	end

	if ksignal == 1 and qdistance - kdistance >= 0 then
		Call("*:SetControlValue", "TrainBrakeControl", 0, 0.9)
		Call("*:LockControl", "TrainBrakeControl", 0, 1)
		Call("*:SetControlValue", "Regulator", 0, 0)
		vlock = 1
...
-- Avisos Aproximacao sinal fechado
	if ksignal == 1 and xdistance - kdistance >= 0 then
		xwarning = 1
		xflash = 1
		sbeep = 1
	else
	...
	if ksignal == 1 and zdistance - kdistance >= 0 then
		rbeep = 1
	end

	if xflash == 1 and ksignal ~= 1 then
		xflash = 0
	end
