...
mTimeSinceLastFlash = 0
mLightFlashOn = false
oTimeSinceLastFlash = 0
oLightFlashOn = false
...
pdistance = (tspeed*time) + pdistance
...
sdistance = (Call("*:GetControlValue", "CNV_Sdistance", 0) - pdistance)
...
--SysCall("ScenarioManager:ShowAlertMessageExt", tonumber(tspeed), tonumber(time), 0.2, 0)
--SysCall("ScenarioManager:ShowAlertMessageExt", tonumber(CNVtest), tonumber(htime), 0.2, 0)
...
-- Convel Inicializacao
htime = htime + time

	if Call("*:GetControlValue", "CNVONOFF", 0) == 1 and ConvelON == 0 and ConvelSTARTING == 0 then
		ConvelSTARTING = 1
		CNVtest = 1
		CNVdisplaycode = 1
	end

	if Call("*:GetControlValue", "CNVONOFF", 0) == 0 then
		ConvelON = 0
	end

	if ConvelSTARTING == 1 then
		
		if CNVtest == 1 then
			if Call("*:GetControlValue", "BrakePipePressureBAR", 0) < 3 then
				CNVcode = 550
			elseif Call("*:GetControlValue", "BrakePipePressureBAR", 0) >= 3 then
				CNVtest = 2
			end
		elseif CNVtest == 2 then
			if Call("*:GetControlValue", "BrakePipePressureBAR", 0) < 4.2 then
				CNVcode = 402
			elseif Call("*:GetControlValue", "BrakePipePressureBAR", 0) >= 4.2 then
				Call("*:SetControlValue", "TrainBrakeControl", 0, 0.9)
				CNVcode = 403
				CNVtest = 3
			end
		elseif CNVtest == 3 then
			if Call("*:GetControlValue", "BrakePipePressureBAR", 0) > 2.5 then
				CNVcode = 403
			elseif Call("*:GetControlValue", "BrakePipePressureBAR", 0) <= 2.5 then
				Call("*:SetControlValue", "TrainBrakeControl", 0, 0)
				CNVtest = 4
				CNVcode = 404
			end
		elseif CNVtest == 4 then
			if Call("*:GetControlValue", "BrakePipePressureBAR", 0) < 4.2 then
				CNVcode = 404
			elseif Call("*:GetControlValue", "BrakePipePressureBAR", 0) >= 4.2 then
				Call("*:SetControlValue", "TrainBrakeControl", 0, 0.9)
		...
		elseif CNVtest == 8 then
			if Call("*:GetControlValue", "BrakePipePressureBAR", 0) < 4.2 then
				CNVcode = 700
			elseif Call("*:GetControlValue", "BrakePipePressureBAR", 0) >= 4.2 then
				Call("*:SetControlValue", "CNV_Test", 0, 1)
				CNVdisplaycode = 0
				errorled = 1
				CNVtest = 9
			end
		elseif CNVtest == 9 then
			if Call("*:GetControlValue", "CNV_Test", 0) == 1 and Call("*:GetControlValue", "CNV_DataEntry", 0) >= 0.5 then
				Call("*:SetControlValue", "CNV_Test", 0, 0)
				errorled = 0
				CNVtest = 10
				htime = 0
			end
		elseif CNVtest == 10 then
			if htime > 3 then
				dataled = 1 
				CNVtest = 11
			end
		elseif CNVtest == 11 then
			if Call("*:GetControlValue", "CNV_DataEntry", 0) >= 0.5 and ((Call("*:GetControlValue", "CNV_rotor04", 0) + Call("*:GetControlValue", "CNV_rotor05", 0)) < 8 or (Call("*:GetControlValue", "CNV_rotor04", 0) + Call("*:GetControlValue", "CNV_rotor05", 0)) > 19) then
				cnv_dataerror = 1
			elseif Call("*:GetControlValue", "CNV_DataEntry", 0) >= 0.5 and ((Call("*:GetControlValue", "CNV_rotor04", 0) + Call("*:GetControlValue", "CNV_rotor05", 0)) >= 8 or (Call("*:GetControlValue", "CNV_rotor04", 0) + Call("*:GetControlValue", "CNV_rotor05", 0)) <= 19) then 
				dataled = 0
				cnv_dataerror = 0
 				ConvelSTARTING = 0
				ConvelON = 1
	...
	if string.len(CNVcode) == 1 then
		CNVcode = ("00" .. CNVcode)
	elseif string.len(CNVcode) == 2 then
		CNVcode = ("0" .. CNVcode)
	end
	_, _, cHundreds, cTens, cUnits = string.find(CNVcode, "(%d)(%d)(%d)")
...
mTimeSinceLastFlash = mTimeSinceLastFlash + time
	
	if errorled == 0 then
		Call("*:SetControlValue", "CNV_ErrorLED", 0, 0)
	elseif errorled == 1 and (not mLightFlashOn) and mTimeSinceLastFlash >= LIGHT_FLASH_OFF_SECS then
		Call("*:SetControlValue", "CNV_ErrorLED", 0, 1)
		mLightFlashOn = true
		mTimeSinceLastFlash = 0
	elseif errorled == 1 and mLightFlashOn and mTimeSinceLastFlash >= LIGHT_FLASH_ON_SECS then
		Call("*:SetControlValue", "CNV_ErrorLED", 0, 0)
		mLightFlashOn = false
		mTimeSinceLastFlash = 0
	end

oTimeSinceLastFlash = oTimeSinceLastFlash + time
	
	if dataled == 0 then
		Call("*:SetControlValue", "CNV_DataentryLED", 0, 0)
	elseif dataled == 1 and (not oLightFlashOn) and oTimeSinceLastFlash >= LIGHT_FLASH_OFF_SECS then
		Call("*:SetControlValue", "CNV_DataentryLED", 0, 1)
		oLightFlashOn = true
		oTimeSinceLastFlash = 0
	elseif dataled == 1 and oLightFlashOn and oTimeSinceLastFlash >= LIGHT_FLASH_ON_SECS then
		Call("*:SetControlValue", "CNV_DataentryLED", 0, 0)
		oLightFlashOn = false
		oTimeSinceLastFlash = 0
	end

	if (block == 1 and tspeed > lspeed) or (slock == 1 and tspeed > bspeed) or (vlock == 1 and tspeed > 1) or (xlock == 1 and tspeed > 5) then
		Call("*:SetControlValue", "CNV_breleaseLED", 0, 1)
	elseif block == 0 and slock == 0 and vlock == 0 and xlock == 0 then
		Call("*:SetControlValue", "CNV_breleaseLED", 0, 0)
	end
...
-- Display Principal
	if cnv_dataerror == 1 then
			Call("*:SetControlValue", "CNV_GUDigits", 0,  10)
			Call("*:SetControlValue", "CNV_GDDigits", 0,  11)
			Call("*:SetControlValue", "CNV_GCDigits", 0,  10)	
	elseif ConvelON == 0 and CNVdisplaycode == 0 then
			Call("*:SetControlValue", "CNV_GUDigits", 0,  10)
			Call("*:SetControlValue", "CNV_GDDigits", 0,  10)
			Call("*:SetControlValue", "CNV_GCDigits", 0,  10)
	elseif ConvelSTARTING == 1 and CNVdisplaycode == 1 then
			Call("*:SetControlValue", "CNV_GUDigits", 0,  tonumber(cUnits))
			Call("*:SetControlValue", "CNV_GDDigits", 0,  tonumber(cTens))
			Call("*:SetControlValue", "CNV_GCDigits", 0,  tonumber(cHundreds))
...
-- Display Auxiliar
	...
	elseif ksignal == 0 and nspeed >= lspeed then
		Call("*:SetControlValue", "CNV_OUDigits", 0,  10)
		Call("*:SetControlValue", "CNV_ODDigits", 0,  10)
		Call("*:SetControlValue", "CNV_OCDigits", 0,  10)
	elseif ksignal == 0 and nspeed < lspeed and ntype ~= 3 then
	...
	if xspeed > (Call("*:GetControlValue", "CNV_rotor01", 0) + Call("*:GetControlValue", "CNV_rotor02", 0))/3.6 then
		lspeed = (Call("*:GetControlValue", "CNV_rotor01", 0) + Call("*:GetControlValue", "CNV_rotor02", 0))/3.6
	else
...
-- Frenagem Sinal Fechado

	if vlock == 1 and Call("*:GetControlValue", "CNV_Brelease", 0) == 1 and tspeed < 1 then
		Call("*:SetControlValue", "TrainBrakeControl", 0, 0)
		Call("*:LockControl", "TrainBrakeControl", 0, 0)
		vlock = 0
		xwarning = 0
...
-- Aproximacao Sinal Fechado

	if ksignal == 1 and dstate == 2 then
		Call("*:SetControlValue", "CNV_Sdistance", 0, ddistance)
		pdistance = 0
		ksignal = 2
	elseif ksignal == 2 and sdistance < xdistance then
		sbeep = 1
		xwarning = 1
		ksignal = 3
	elseif ksignal == 3 and sdistance < zdistance then
		rbeep = 1
		ksignal = 4
	elseif ksignal == 4 and sdistance < qdistance then
		Call("*:SetControlValue", "TrainBrakeControl", 0, 0.9)
		Call("*:LockControl", "TrainBrakeControl", 0, 1)
		Call("*:SetControlValue", "Regulator", 0, 0)
		xlock = 1
	...
	if (ksignal == 4 or (ksignal == 3 and sdistance < zdistance) or (ksignal == 2 and sdistance < xdistance)) and tspeed < 4.17 then
		ksignal = 5
	end
	
	if ksignal == 5 and tspeed > 4.17 then
		Call("*:SetControlValue", "CNV_Overspeed", 0, 1)
	elseif ksignal == 5 and tspeed <= 4.17 then
		Call("*:SetControlValue", "CNV_Overspeed", 0, 0)
	end

	if ksignal == 5 and tspeed > 5 then
		Call("*:SetControlValue", "TrainBrakeControl", 0, 0.9)
		Call("*:LockControl", "TrainBrakeControl", 0, 1)
		Call("*:SetControlValue", "Regulator", 0, 0)
		xlock = 1
	...
	if xlock == 1 and Call("*:GetControlValue", "CNV_Brelease", 0) == 1 and tspeed < 5 then
		Call("*:SetControlValue", "TrainBrakeControl", 0, 0)
		Call("*:LockControl", "TrainBrakeControl", 0, 0)
		xlock = 0
	end
